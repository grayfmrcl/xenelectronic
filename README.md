...
Navigate to /client and install `gh-pages`

    npm install --save-dev gh-pages

Modify `script` section in `package.json` by adding the following

    predeploy: "npm run build"
    deploy: "gh-pages -d build"

Before we build and deploy the client app, change the `API_URL` in the `.env` file to the url generated by the heroku before.

At last, deploy the client.

    npm run deploy
    
Deployed application can be accessed here: https://grayfmrcl.github.io/xenelectronic/
